...
def startChess(message):
    ...
    if len(users_chess) % 2 != 0:
        users_chess['fake'] = -1
    for user in range(0, len(users_chess), 2 ):
        text = "Вы будете играть с" + user+1
        bot.send_message(user, str)
        text = "Вы будете играть с" + user
        bot.send_message(user+1, str)
...
def startTennis(message):
    ...
    if len(users_chess) % 2 != 0:
        users_chess['fake'] = -1
    for i in range(0, len(users_tennis), 2 ):
        text = "Вы будете играть с" + str(users_tennis[i+1])
        bot.send_message(users_tennis[i], str)
        text = "Вы будете играть с" + str(users_tennis[i+1])
        bot.send_message(users_tennis[i], str)
